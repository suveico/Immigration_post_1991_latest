...
total_emmigr_total_data = df.loc[:, [
     f"Unnamed: {n}" for n in range(4,28,6)]].iloc[2:3].astype('int32')

total_emmigr_men_data = df.loc[:, [
     f"Unnamed: {n}" for n in range(5, 29, 6)]].iloc[2:3].astype('int32')

total_emmigr_women_data = df.loc[:, [
     f"Unnamed: {n}" for n in range(6, 30, 6)]].iloc[2:3].astype('int32')
...
############ PREPARE  ############################
############ PLOT & SAVE #########################
output_file(filepath + "__4-B.html")

men = total_emmigr_men_data.values.tolist()[0]
